...
const DEFAULT_PULSE_CREDENTIALS = {
    userId: '',
    password: ''
};
...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'pulse-config');
    } else if (platform === 'darwin') {
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'pulse-config');
    } else {
        configDir = path.join(os.homedir(), '.config', 'pulse-config');
    }
...
function getPulseCredentialsPath() {
    return path.join(getConfigDir(), 'pulse-credentials.json');
}
