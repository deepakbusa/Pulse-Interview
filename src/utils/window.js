...
function createWindow(sendToRenderer, geminiSessionRef) {
    ...
    // Handle permissions for media devices (system audio capture)
    ...
    // Handle display media requests for screen and system audio capture
    session.defaultSession.setDisplayMediaRequestHandler(
        (request, callback) => {
            desktopCapturer.getSources({ types: ['screen', 'window'] }).then(sources => {
                // Grant access to screen and system audio (loopback)
                if (sources.length > 0) {
                    callback({ 
                        video: sources[0], 
...
function updateGlobalShortcuts(keybinds, mainWindow, sendToRenderer, geminiSessionRef) {
    ...
    // Register send transcription shortcut (Ctrl+D)
    if (keybinds.sendTranscription) {
        try {
            globalShortcut.register(keybinds.sendTranscription, () => {
                console.log('Send transcription shortcut triggered (Ctrl+D)');
                sendToRenderer('send-transcription');
            });
...
function setupWindowIpcHandlers(mainWindow, sendToRenderer, geminiSessionRef) {
    ...
    ipcMain.handle('update-sizes', async event => {
        try {
            ...
            try {
                viewName = await event.sender.executeJavaScript('pulse.getCurrentView()');
                layoutMode = await event.sender.executeJavaScript('pulse.getLayoutMode()');
            } catch (error) {
